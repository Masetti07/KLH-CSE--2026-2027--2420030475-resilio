...
export default function Workspace() {
  ...
  const loadDesignState = async (planId: string, currentStructure: Structure, recoveredConfiguration?: DesignConfiguration) => {
    ...
    } catch (reason) { if (generation === planGeneration.current) setDesignError(reason instanceof Error ? reason.message : "Design versions could not be loaded."); }
    finally { if (generation === planGeneration.current) setDesignBusy(false); }
  };
  ...
  const chooseFile = (file?: File) => {
    ...
    resetPlan(); setSelectedFile(file); setPreviewUrl(URL.createObjectURL(file)); setStatus("selected"); setStartView("upload");
  };
  ...
  const process = async (file = selectedFile) => {
    ...
    try { await acceptPlan(await uploadPlan(file), generation); }
