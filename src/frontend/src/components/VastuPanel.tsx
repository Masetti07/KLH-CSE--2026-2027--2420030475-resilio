...
import { VASTU_DISCLAIMER, visibleVastuResults } from "../utils/vastuAssist";
...
export default function VastuPanel({ structure, design, configuration, analysis, busy, showZones, onShowZones, onConfigurationChange, onRun }: Props) {
  ...
  const visibleResults = visibleVastuResults(analysis, configuration, structure);
  ...
  return <div className="vastu-panel">
    ...
    <div className="vastu-results">
      ...
      {analysis && <><div className="analysis-counts"><div><strong>{analysis.counts.satisfied}</strong><span>Satisfied</span></div><div><strong>{analysis.counts.unsatisfied}</strong><span>Unsatisfied</span></div><div><strong>{analysis.counts.cannot_evaluate}</strong><span>Cannot Evaluate</span></div></div>{analysis.warnings.map((warning) => <p className="analysis-warning" key={warning}>{warning}</p>)}{visibleResults.length === 0 && <p role="status">No configured directional rules apply to the assigned rooms.</p>}<div className="rule-results">{visibleResults.map((result, index) => <article key={`${result.rule_id}-${result.room_id ?? index}`} className={`rule-card ${result.result}`}><div><span>{result.rule_id}</span><strong>{result.rule_title}</strong><em>{result.result.replaceAll("_", " ")}</em></div><p>{result.explanation}</p><small>Room: {result.room_name ?? result.room_type.replaceAll("_", " ")} · Detected: {result.detected_zone?.replaceAll("_", "-") ?? "Unavailable"} · Preferred: {result.preferred_zones.map((zone) => zone.replaceAll("_", "-")).join(", ")}</small></article>)}</div></>}
      <p className="vastu-disclaimer compact">{VASTU_DISCLAIMER}</p>
    </div>
  </div>;
}
