...
type Props = { structure: Structure; designs: Design[]; designAId: string; designBId: string; visualId: string; selection: Selection; fullscreen: boolean; renderingQuality?: "NORMAL" | "PERFORMANCE"; rendererAvailable?: boolean; onTelemetry?: (fps: number, frameTime: number) => void; onFailure?: () => void; onDesignA: (id: string) => void; onDesignB: (id: string) => void; onVisual: (id: string) => void; onSelect: (selection: Selection) => void; onToggleFullscreen: () => void; onReturnTo2D: () => void };

export default function ComparePanel({ structure, designs, designAId, designBId, visualId, selection, fullscreen, renderingQuality, rendererAvailable, onTelemetry, onFailure, onDesignA, onDesignB, onVisual, onSelect, onToggleFullscreen, onReturnTo2D }: Props) {
  ...
  return <div className="compare-panel">
    ...
    <div className="comparison-view"><span className="active-design-badge">Viewing: {visual.name}</span><RendererBoundary onFailure={onFailure} onReturnTo2D={onReturnTo2D}><ThreeViewer structure={structure} design={visual.configuration} selection={selection} onSelect={onSelect} fullscreen={fullscreen} onToggleFullscreen={onToggleFullscreen} renderingQuality={renderingQuality} available={rendererAvailable} onTelemetry={onTelemetry} /></RendererBoundary></div>
